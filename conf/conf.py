...
def re_open():
    installed = True if os.path.exists("/usr/local/bin/AutoPent") else False

    if installed:
        os.system("sudo AutoPent")
        sys.exit()

    else:
        os.system("sudo python3 AutoPent.py")
        sys.exit(())
    ...
    try:
        ...
        if ver_sc == 200:
            ...
            if version == github_ver:
                print(
                    colored(
                        "Your AutoPent version is Up-To-Date\n",
                        "yellow",
                        attrs=["reverse"],
                    ))
            else:
                print(
                    colored(
                        f"Your AutoPent version is Out-Dated, New _version Available: {format(github_ver)} \n",
                        "red",
                        attrs=["reverse"],
                    ))
        else:
